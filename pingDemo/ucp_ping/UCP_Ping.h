...
@protocol UCP_PingDelegate;

/*! Controls the IP address version used by UCP_Ping instances.
 */

typedef NS_ENUM(NSInteger, UCP_PingAddressStyle) {
    UCP_PingAddressStyleAny,          ///< Use the first IPv4 or IPv6 address found; the default.
    UCP_PingAddressStyleICMPv4,       ///< Use the first IPv4 address found.
    UCP_PingAddressStyleICMPv6        ///< Use the first IPv6 address found.
};
 ...
 */
@property (nonatomic, weak, readwrite, nullable) id<UCP_PingDelegate> delegate;

/*! Controls the IP address version used by the object.
 *  \details You should set this value before starting the object.
 */
@property (nonatomic, assign, readwrite) UCP_PingAddressStyle addressStyle;
...
/*! A delegate protocol for the UCP_Ping class.
 */

@protocol UCP_PingDelegate <NSObject>

@optional

/*! A UCP_Ping delegate callback, called once the object has started up.
...
/*! A UCP_Ping delegate callback, called if the object fails to start up.
 *  \details This is called shortly after you start the object to tell you that the 
...
/*! A UCP_Ping delegate callback, called when the object has successfully sent a ping packet.
...
/*! A UCP_Ping delegate callback, called when the object fails to send a ping packet.
 *  \details Each call to `-sendPingWithData:` will result in either a 
 *      `-ping:didSendPacket:sequenceNumber:` delegate callback or a
 *      `-ping:didFailToSendPacket:sequenceNumber:error:` delegate callback (unless you
 *      stop the object before you get the callback).  These callbacks are currently delivered 
...
/*! A UCP_Ping delegate callback, called when the object receives a ping response.
...
/*! A UCP_Ping delegate callback, called when the object receives an unmatched ICMP message.
 ...
 *  
 *      This is declared in the header because clients of UCP_Ping might want to use
