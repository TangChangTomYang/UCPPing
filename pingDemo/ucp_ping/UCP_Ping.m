...
- (void)didFailWithError:(NSError *)error {
    id<UCP_PingDelegate>  strongDelegate;
    ...
    if ( (strongDelegate != nil) && [strongDelegate respondsToSelector:@selector(ping:didFailWithError:)] ) {
        [strongDelegate ping:self didFailWithError:error];
    }
}
...
- (void)sendPingWithData:(NSData *)data {
    ...
    // data may be nil
    NSParameterAssert(self.hostAddress != nil);     // gotta wait for -ping:didStartWithAddress:
    ...
    if ( (bytesSent > 0) && (((NSUInteger) bytesSent) == packet.length) ) {
        ...
        if ( (strongDelegate != nil) && [strongDelegate respondsToSelector:@selector(ping:didSendPacket:sequenceNumber:)] ) {
            [strongDelegate ping:self didSendPacket:packet sequenceNumber:self.nextSequenceNumber];
        }
    } else {
        NSError *   error;
        ...
        if ( (strongDelegate != nil) && [strongDelegate respondsToSelector:@selector(ping:didFailToSendPacket:sequenceNumber:error:)] ) {
            [strongDelegate ping:self didFailToSendPacket:packet sequenceNumber:self.nextSequenceNumber error:error];
        }
    }
...
- (void)readData {
    ...
    if (bytesRead > 0) {
        ...
        if ( [self validatePingResponsePacket:packet sequenceNumber:&sequenceNumber] ) {
            if ( (strongDelegate != nil) && [strongDelegate respondsToSelector:@selector(ping:didReceivePingResponsePacket:sequenceNumber:)] ) {
                [strongDelegate ping:self didReceivePingResponsePacket:packet sequenceNumber:sequenceNumber];
            }
        } else {
            if ( (strongDelegate != nil) && [strongDelegate respondsToSelector:@selector(ping:didReceiveUnexpectedPacket:)] ) {
                [strongDelegate ping:self didReceiveUnexpectedPacket:packet];
            }
        }
    } else {
...
static void SocketReadCallback(CFSocketRef s, CFSocketCallBackType type, CFDataRef address, const void *data, void *info) {
    ...
    obj = (__bridge UCP_Ping *) info;
    assert([obj isKindOfClass:[UCP_Ping class]]);
...
- (void)startWithHostAddress {
    ...
    if (err != 0) {
        ...
        strongDelegate = self.delegate;
        if ( (strongDelegate != nil) && [strongDelegate respondsToSelector:@selector(ping:didStartWithAddress:)] ) {
            [strongDelegate ping:self didStartWithAddress:self.hostAddress];
        }
    }
    assert(fd == -1);
}
        ...
        for (NSData * address in addresses) {
            ...
            if ( address.length >= sizeof(struct sockaddr) ) {
                switch (addrPtr->sa_family) {
                    case AF_INET: {
                        if (self.addressStyle != UCP_PingAddressStyleICMPv6) {
                            self.hostAddress = address;
                            resolved = true;
                        }
                    } break;
                    case AF_INET6: {
                        if (self.addressStyle != UCP_PingAddressStyleICMPv4) {
                            self.hostAddress = address;
                            resolved = true;
                        }
                    } break;
